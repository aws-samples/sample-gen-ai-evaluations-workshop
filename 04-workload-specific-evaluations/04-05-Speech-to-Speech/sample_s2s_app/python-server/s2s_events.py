...
class S2sEvent:
  ...
  @staticmethod
  def text_input(prompt_name, content_name, system_prompt=DEFAULT_SYSTEM_PROMPT, role=None):
    text_input_event = {
      "event":{
        "textInput":{
          "promptName":prompt_name,
    ...
    # Add role if specified (e.g., "SYSTEM" for system prompts)
    if role:
      text_input_event["event"]["textInput"]["role"] = role
