...
import './s2s.css'
import { Icon, Alert, Button, Modal, Box, SpaceBetween, FormField, Select, Textarea, Input, Container, Header, Badge } from '@cloudscape-design/components';
...
import { DemoProfiles, Voices, ModelConfig } from './helper/config';

// Model IDs that support text input
const MODELS_WITH_TEXT_INPUT = ['amazon.nova-2-sonic-v1:0'];
...
class S2sChatBot extends React.Component {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            selectedDemoProfileOption: DemoProfiles && DemoProfiles.length > 1 ? {
                label: DemoProfiles[1].name,
                value: DemoProfiles[1].name,
                description: DemoProfiles[1].description
            } : DemoProfiles ? {
            ...
            configModelId: "amazon.nova-2-sonic-v1:0",
            ...
            configInferenceConfig: S2sEvent.DEFAULT_INFER_CONFIG,
    ...
    saveStateToStorage = () => {
        const stateToSave = {
            ...
            configModelId: this.state.configModelId,
            ...
            configInferenceConfig: this.state.configInferenceConfig,
            enableParalinguisticDetection: this.state.enableParalinguisticDetection,
            enableRecordAudio: this.state.enableRecordAudio
        };
        ...
        try {
            ...
            if (savedState) {
                ...
                this.setState({
                    ...
                    configModelId: parsedState.configModelId || this.state.configModelId,
                    ...
                    configInferenceConfig: parsedState.configInferenceConfig || this.state.configInferenceConfig,
                    enableParalinguisticDetection: parsedState.enableParalinguisticDetection || false,
                    enableRecordAudio: parsedState.enableRecordAudio || false
                });
    ...
    // Helper method to check if chat history is valid and should be included
    shouldIncludeChatHistory = () => {
        try {
            const chatHistory = JSON.parse(this.state.configChatHistory);

            // Return false if chatHistory is null, undefined, or not an array
            ...
            // Return false if the array contains only empty objects like [{}]
            if (chatHistory.length === 1 &&
                typeof chatHistory[0] === 'object' &&
                Object.keys(chatHistory[0]).length === 0) {
                return false;
            }

            // Return true if we have valid chat history with content
            return true;
        } catch (error) {
    ...
    connectWebSocket() {
        ...
        if (this.socket === null || this.socket.readyState !== WebSocket.OPEN) {
            ...
            const ws_url = process.env.REACT_APP_WEBSOCKET_URL ? process.env.REACT_APP_WEBSOCKET_URL : "ws://localhost:8081/ws"
            this.socket = new WebSocket(ws_url);
            this.socket.onopen = () => {
                console.log("WebSocket connected!");

                // Send model configuration first
                this.sendEvent({ event: { modelConfig: { modelId: this.state.configModelId } } });

                // Send recording configuration
                this.sendEvent({ event: { recordingConfig: { enableRecording: this.state.enableRecordAudio } } });

                // sessionStart - check if model supports turn sensitivity
                const currentModel = ModelConfig[this.state.configModelId];
                // Use configurable inference config instead of DEFAULT_INFER_CONFIG
                const sessionStartEvent = S2sEvent.sessionStart(this.state.configInferenceConfig, this.state.configTurnSensitivity);

                // Remove turnDetectionConfiguration if model doesn't support it
                if (!currentModel?.supportsTurnSensitivity && sessionStartEvent.event.sessionStart.turnDetectionConfiguration) {
                    delete sessionStartEvent.event.sessionStart.turnDetectionConfiguration;
                }
                ...
                // Only send if model supports it AND user has enabled it
                if (this.state.enableParalinguisticDetection && currentModel?.supportsParalinguisticDetection) {
                ...
                // Chat history - automatically include if valid
                if (this.shouldIncludeChatHistory()) {
    ...
    handleDemoProfileChange = e => {
        ...
        if (demoProfile) {
            this.setState({
                ...
                configModelId: demoProfile.modelId || this.state.configModelId,
                configSystemPrompt: demoProfile.systemPrompt,
                configToolUse: JSON.stringify(demoProfile.toolConfig, null, 4),
                configInferenceConfig: demoProfile.inferenceConfig || this.state.configInferenceConfig,
            }, () => {
                ...
                <div className='header'>
                    <div className='header-content'>
                        <div className='app-title'>
                            <h1>Amazon Nova Sonic Test Playground</h1>
                        </div>
                    ...
                    <div className="content-columns">
                        <div className="conversation-section">
                            ...
                            <div className="conversation-container">
                                ...
                                {/* Text input - only show for models that support it */}
                                {MODELS_WITH_TEXT_INPUT.includes(this.state.configModelId) && (
