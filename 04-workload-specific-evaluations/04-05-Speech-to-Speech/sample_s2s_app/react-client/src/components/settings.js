...
import { Modal, Box, SpaceBetween, Button, FormField, Select, Textarea, Container, Header, ColumnLayout, Badge, Input } from '@cloudscape-design/components';
import { VoicesByLanguage, ModelConfig } from '../helper/config';
...
class Settings extends React.Component {
    constructor(props) {
        ...
        this.state = {
            ...
            configModelId: props.configModelId,
            ...
            configInferenceConfig: props.configInferenceConfig,
            enableParalinguisticDetection: props.enableParalinguisticDetection,
            enableRecordAudio: props.enableRecordAudio
        };
    }
    ...
    componentDidUpdate(prevProps) {
        ...
        if (prevProps.configVoiceIdOption !== this.props.configVoiceIdOption ||
            prevProps.configModelId !== this.props.configModelId ||
            ...
            prevProps.configInferenceConfig !== this.props.configInferenceConfig ||
            prevProps.enableParalinguisticDetection !== this.props.enableParalinguisticDetection ||
            prevProps.enableRecordAudio !== this.props.enableRecordAudio) {
            this.setState({
                configVoiceIdOption: this.props.configVoiceIdOption,
                configModelId: this.props.configModelId,
                ...
                configInferenceConfig: this.props.configInferenceConfig,
                enableParalinguisticDetection: this.props.enableParalinguisticDetection,
                enableRecordAudio: this.props.enableRecordAudio
            });
        }
    }
    ...
    handleRecordAudioChange = (e) => {
        const enabled = e.target.checked;
        this.setState({ enableRecordAudio: enabled });
        if (this.props.onSettingsChange) {
            this.props.onSettingsChange({ enableRecordAudio: enabled });
        }
    }

    handleInferenceConfigChange = (field, value) => {
        const updatedConfig = {
            ...this.state.configInferenceConfig,
            [field]: value
        };
        this.setState({ configInferenceConfig: updatedConfig });
        if (this.props.onSettingsChange) {
            this.props.onSettingsChange({ configInferenceConfig: updatedConfig });
        }
    }
        ...
        return (
            ...
            >
                <SpaceBetween size="l">
                    ...
                    >
                        <SpaceBetween size="m">
                            {/* Get current model config from props */}
                            {(() => {
                                const currentModel = ModelConfig[this.props.configModelId];
                                const columnCount = (currentModel?.supportsTurnSensitivity ? 1 : 0) + 1;

                                return (
                                    <ColumnLayout columns={Math.min(columnCount, 2)} variant="text-grid">
                                        <FormField
                                            label={
                                        ...
                                        {/* Turn Sensitivity - only show if model supports it */}
                                        {currentModel?.supportsTurnSensitivity && (
                                            <FormField
                                                label={
                                                    <SpaceBetween direction="horizontal" size="xs">
                            ...
                            {/* Paralinguistic Detection - only show if model supports it */}
                            {(() => {
                                const currentModel = ModelConfig[this.props.configModelId];
                                return currentModel?.supportsParalinguisticDetection ? (
                                    <FormField
                                        label={
                                            <SpaceBetween direction="horizontal" size="xs">
                                                <span>Paralinguistic Detection</span>
                                                <Badge color="blue">Sentiment</Badge>
                                            </SpaceBetween>
                                        }
                                        description="Enable sentiment and emotion detection in user speech"
                                        stretch={true}
                                    >
                                        <div style={{ padding: '8px 0' }}>
                                            <label style={{ display: 'flex', alignItems: 'center', cursor: 'pointer' }}>
                                                <input
                                                    type="checkbox"
                                                    checked={this.state.enableParalinguisticDetection}
                                                    onChange={this.handleParalinguisticDetectionChange}
                                                    style={{ marginRight: '8px', cursor: 'pointer' }}
                                                />
                                                <span>Enable sentiment detection</span>
                                            </label>
                                        </div>
                                    </FormField>
                                ) : null;
                            ...
                            <FormField
                                label={
                                    <SpaceBetween direction="horizontal" size="xs">
                                        <span>Record Audio Interaction</span>
                                        <Badge color="red">Recording</Badge>
                                    </SpaceBetween>
                                }
                                description="Record microphone input and audio output to WAV files (saved with session ID)"
                                stretch={true}
                            >
                                <div style={{ padding: '8px 0' }}>
                                    <label style={{ display: 'flex', alignItems: 'center', cursor: 'pointer' }}>
                                        <input
                                            type="checkbox"
                                            checked={this.state.enableRecordAudio}
                                            onChange={this.handleRecordAudioChange}
                                            style={{ marginRight: '8px', cursor: 'pointer' }}
                                        />
                    ...
                    <Container
                        header={
                            <Header
                                variant="h2"
                                description="Control the model's response generation parameters"
                            >
                                Inference Configuration
                            </Header>
                        }
                    >
                        <ColumnLayout columns={3} variant="text-grid">
                            <FormField
                                label={
                                    <SpaceBetween direction="horizontal" size="xs">
                                        ...
                                        <Badge color="blue">Generation</Badge>
                                    </SpaceBetween>
                                }
                            ...
                            >
                                <Input
                                    type="number"
                                    value={String(this.state.configInferenceConfig?.maxTokens || 1024)}
                                    onChange={({ detail }) => {
                                        const value = parseInt(detail.value) || 1024;
                                        this.handleInferenceConfigChange('maxTokens', value);
                                    }}
                                    placeholder="1024"
                                />
                            </FormField>
                            ...
                            <FormField
                                label={
                                    <SpaceBetween direction="horizontal" size="xs">
                                        ...
                                        <Badge color="green">Creativity</Badge>
                                    </SpaceBetween>
                                }
                            ...
                            >
                                <Input
                                    type="number"
                                    value={String(this.state.configInferenceConfig?.temperature || 0.7)}
                                    onChange={({ detail }) => {
                                        const value = parseFloat(detail.value) || 0.7;
                                        this.handleInferenceConfigChange('temperature', value);
                                    }}
                                    placeholder="0.7"
                                    step={0.1}
                                />
                            ...
                            <FormField
                                label={
                                    <SpaceBetween direction="horizontal" size="xs">
                                        <span>Top P</span>
                                        <Badge color="purple">Diversity</Badge>
                                    </SpaceBetween>
                                }
                            ...
                            >
                                <Input
                                    type="number"
                                    value={String(this.state.configInferenceConfig?.topP || 0.95)}
                                    onChange={({ detail }) => {
                                        const value = parseFloat(detail.value) || 0.95;
                                        this.handleInferenceConfigChange('topP', value);
                                    }}
                                    placeholder="0.95"
                                    step={0.05}
                                />
